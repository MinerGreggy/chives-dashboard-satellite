...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'chives-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('chives').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('chives')[service]);
